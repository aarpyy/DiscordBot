...
def main(bnet):
    # If couldn't find N_HEROES, raise error so program can't continue
    if not isinstance(config.N_HEROES, int):
        raise FileNotFoundError('config file missing from Formatter/config.h')
    else:
        N_HEROES = config.N_HEROES
    ...
    # For whatever reason on replit the sed can't pipe stdout to format, so it prints stdout to file that is then read by format
    ...
    db['{0}#{1}'.format(player_name, player_id)] = {'ranks': ranks, 'time-played': time_played}
